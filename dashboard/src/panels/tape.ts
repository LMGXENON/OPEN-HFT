...
export class TapePanel extends Panel {
  ...
  render(c: RenderCtx): void {
    ...
    this.renderLive(tradeList, n);
  }

  renderLive(trades: Array<{ time: number; price: number; qty: number; side: "BUY" | "SELL"; cpty?: string }>, totalCount?: number): void {
    ...
    const key = `${count}|${trades[0]?.time ?? 0}|${trades[0]?.price ?? 0}|${trades[0]?.cpty ?? ""}|${cols}|${rows}`;
    ...
    this.setTitle(`${count.toLocaleString()} trades │ vol ${volStr} │ tape stream`);
  }
}
