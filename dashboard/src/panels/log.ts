...
import { sp } from "../dom";
import { clock, dur, fmtSmartPrice, fmtSmartQty, lj, lotDigits, ms, tickDigits } from "../fmt";
...
export class LogPanel extends Panel {
  ...
  renderLive(events: Array<{ timeStr: string; type: string; side: "BUY" | "SELL"; price: number; qty: number; detail: string; cls: string }>): void {
    const rows = this.rows;
    const out: string[] = [];
    ...
    const key = `${events.length}|${events[events.length - 1]?.timeStr ?? ""}`;
    if (!this.changed(key)) return;

    const recent = events.slice(-rows).reverse();
    for (const ev of recent) {
      const sideCls = ev.side === "BUY" ? "bid" : "ask";
      const side = ev.side === "BUY" ? "BUY " : "SELL";
      const px = fmtSmartPrice(ev.price);
      const qty = fmtSmartQty(ev.qty);
      const kcls = ev.type === "FILL" ? "y" : ev.type === "ACK" ? "g" : (ev.type === "NEW" || ev.type === "SUBMIT") ? "w" : ev.type === "TOUCH" ? "c" : ev.type === "CXLD" ? "d" : "r";
      const isNewOrFill = ev.type === "NEW" || ev.type === "SUBMIT" || ev.type === "FILL" || ev.type === "ACK";
      const qtyStr = isNewOrFill ? qty : " ".repeat(qty.length);
      const line =
        sp("d", ev.timeStr) +
        " " +
        sp(kcls, lj(ev.type, 6)) +
        " " +
        sp(sideCls, side) +
        ` ${qtyStr} @ ${sp("ours", px)}` +
        " " +
