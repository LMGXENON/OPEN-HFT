...
export function fmtSmartPrice(p: number): string {
...
export function fmtSmartQty(q: number): string {
  if (!Number.isFinite(q) || q === 0) return "--";
  if (q >= 1_000_000_000) return (q / 1e9).toFixed(2) + "B";
  if (q >= 1_000_000) return (q / 1e6).toFixed(2) + "M";
  if (q >= 10_000) return (q / 1e3).toFixed(1) + "k";
